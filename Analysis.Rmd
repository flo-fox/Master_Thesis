...
rdd <- rdrobust::rdrobust(y = d_rdd_bivariate$ln_gross_expenditure_pc, x = d_rdd_bivariate$inhabs_rel_to_cutoff, fuzzy = d_rdd_bivariate$total_seats)
summary(rdd)
...
**Hier sollten vermutlich nur Wahljahre auftauchen, sonst doppelte Fälle!**

McCrary test for all states pooled:
```{r}
rdd::DCdensity(runvar = data$inhabs_rel_to_cutoff, cutpoint = 0, plot = TRUE)
rdd::DCdensity(runvar = data$inhabs_rel_to_cutoff, cutpoint = 0, plot = FALSE, verbose = TRUE)
```
```{r}
rdd::DCdensity(runvar = d_rdd_bivariate$inhabs_rel_to_cutoff, cutpoint = 0, plot = TRUE)
rdd::DCdensity(runvar = d_rdd_bivariate$inhabs_rel_to_cutoff, cutpoint = 0, plot = FALSE, verbose = TRUE)
```
...
How does estimate react to different bandwidth?
...
data2 <- data %>%
  ...
  drop_na() %>%
  slice(50:500)
rdd <- rdmulti::rdms(Y = data2$gross_expenditure, X = data2$exact_pop, C = unique(data2$inhabitants_treshold), fuzzy = data2$total_seats)
```
...
data2 <- data %>%
  ...
  drop_na() %>%
  slice(50:500)
rdrobust::rdrobust(y = data2$gross_expenditure, x = data2$exact_pop, c = 0, fuzzy = data2$total_seats)
```
