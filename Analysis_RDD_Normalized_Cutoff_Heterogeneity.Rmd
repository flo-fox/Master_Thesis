...
## How is the additional spending financed?
...
## If taxes are used to fund the Law, which taxes?
...
rdd_tex_export <- tibble(
  ...
  `Ln of gross revenue p. c.` = c("FE RDD w/o covariates", "(1)", round(rdd_h2_taxes$Estimate[1], round_decimals), paste0("(", round(rdd_h2_taxes$se[1], round_decimals), ")"), paste0("[", round(rdd_h2_taxes$ci[3,1], 3), ", ", round(rdd_h2_taxes$ci[3,2], 3), "]"), paste0(p_value(rdd_h2_taxes$pv[3]), assign_stars(rdd_h2_taxes$pv[3])), "All data", sum(rdd_h2_taxes$N), round(rdd_h2_taxes$bws[1,1], round_decimals), rdd_h2_taxes$N_h[1], rdd_h2_taxes$N_h[2]),
  `Ln of net expenditure p. c.` = c("FE RDD w/o covariates", "(2)", round(rdd_h3_debt$Estimate[1], round_decimals), paste0("(", round(rdd_h3_debt$se[1], round_decimals), ")"), paste0("[", round(rdd_h3_debt$ci[3,1], 3), ", ", round(rdd_h3_debt$ci[3,2], 3), "]"), paste0(p_value(rdd_h3_debt$pv[3]), assign_stars(rdd_h3_debt$pv[3])), "All data", sum(rdd_h3_debt$N), round(rdd_h3_debt$bws[1,1], round_decimals), rdd_h3_debt$N_h[1], rdd_h3_debt$N_h[2])
  )
rdd_tex_export
```
...
print(
  ...
  booktabs = TRUE,
  floating = FALSE,
  hline.after = c(-1, -1, 2, 6, nrow(rdd_tex_export), nrow(rdd_tex_export)),
...
plot_states_fuzzy1 <- ggplot2::ggplot(data = df, aes(x = state_name, y = point_estimate)) +
  ...
  geom_errorbar(aes(ymin = ci_lower, ymax = ci_upper),
                ...
                width = 0.5) +
  geom_text(aes(y = -1.75, label = note),
            colour = ifelse(df$p <= p_value_cutoff, 2, 1), vjust = 0, size = 2.8) +
  geom_hline(aes(yintercept = 0), linetype = "dotted") +
  labs(x = "State", y = "Increase in council size at the thresholds")
plot_states_fuzzy1
...
plot_states_fuzzy2 <- ggplot2::ggplot(data = df, aes(x = state_name, y = point_estimate)) +
  ...
  geom_errorbar(aes(ymin = ci_lower, ymax = ci_upper),
                ...
                linetype = ifelse(df$p <= p_value_cutoff, 2, 1),
                width = 0.5) +
  geom_text(aes(y = -0.22, label = note),
            colour = ifelse(df$p <= p_value_cutoff, 2, 1), vjust = 0, size = 2.8) +
  geom_hline(aes(yintercept = 0), linetype = "dotted") +
  scale_y_continuous(labels = scales::percent) +
  labs(x = "State", y = y_lab_RDD)
plot_states_fuzzy2
...
plot_states_sharp <- ggplot2::ggplot(data = df, aes(x = state_name, y = point_estimate)) +
  ...
  geom_errorbar(aes(ymin = ci_lower, ymax = ci_upper),
                ...
                linetype = ifelse(df$p <= p_value_cutoff, 2, 1),
                width = 0.3) +
  geom_text(aes(y = -0.165, label = note),
...
Interaction-term method in the *subsample* approach:

```{r}
all_states <- data %>%
  distinct(state) %>%
  filter(state != "3", state != "12", state != "13", state != "15") %>%
  pull() # vector of states
df <- data %>%
  filter(closest <= 0.1) %>%
  mutate(state = as.factor(state))
for (i in all_states) {
  df_state <- df %>% filter(state == i)
  rdd <- fixest::feols(ln_gross_expenditure_pc ~
                       1  | # controls
...
States 13 (MV) and 15 (ST) apparently not testable...
...
## At-large vs. ward electoral systems

Dummy `ward_elections` is 1 for states 01 and 05, 0 otherwise. 01 & 05 have mixed elections.
...
ggplot2::ggplot(data = df, aes(x = value2, y = point_estimate)) +
  geom_point(size = 1.5, color = ifelse(df$p <= p_value_cutoff, 2, 1)) +
  geom_errorbar(aes(ymin = ci_lower, ymax = ci_upper),
                color = ifelse(df$p <= p_value_cutoff, 2, 1),
                linetype = ifelse(df$p <= p_value_cutoff, 2, 1),
                width = 0.3) +
  geom_text(aes(y = -0.0375, label = note),
            colour = ifelse(df$p <= p_value_cutoff, 2, 1), vjust = 0) +
  geom_hline(aes(yintercept = 0), linetype = "dotted") +
  scale_y_continuous(labels = scales::percent) +
  labs(x = NULL, y = y_lab_RDD)
ggsave(filename = "plots/rdd_atlarge_mixed.pdf", width = 6, height = 6*9/16)
rm(df)
```
...
Hard to pin down local constitutional types. The way I did it:

- `const_typ_sueddt` = 1 if states: 01, 03, 05, 07, 08, 09, 10, 14, 0 otherwise. I. e. = 1 if "pure" "süddeutsche Ratsverfassung".
- `const_typ_magistrat` = 1 if state: 06, 0 otherwise. Only Hesse has pure "Magistratsverfassung".

### `const_typ_sueddt` -- Süddeutsche Ratsverfassung

(Main specification)

Subsample method:

```{r, rdd_const_typ_sueddt, eval=FALSE}
# to do: remove eval=FALSE
data_subset <- data %>% # prepare data set used for analysis
...
### `const_typ_magistrat` -- Magistratsverfassung

We should expect `const_typ_magistrat == 1` to be significant as it only consists of Hesse, which had a large positive effect in the state-heterogeneity section (subsection method).

Subsample method:

```{r, rdd_const_typ_magistrat, eval=FALSE}
# to do: remove eval=FALSE
data_subset <- data %>% # prepare data set used for analysis
  select(ln_gross_expenditure_pc, inhabs_rel_to_cutoff, total_seats, inhabitants_treshold_factor, state, year, const_typ_magistrat) %>%
  drop_na()
all_values <- data_subset %>% distinct(const_typ_magistrat) %>% pull() # vector of values
rdd_vary_const_type2 <- # prepare data set to store the results
  tibble(const_typ_magistrat = "", point_estimate = "", se = "", se_robust = "",
         ci_lower = "", ci_upper = "", p = "", bw = "", obs_below = "", obs_above = "") %>%
  mutate(across(everything(), as.numeric))
for (i in all_values) {
  print(paste("constitutional type:", i))
  df <- data_subset %>%
    filter(const_typ_magistrat == i)
  rdd <-
    rdrobust_wrapper(
      ccluster = df$inhabitants_treshold_factor,
      ccovs = model.matrix( ~ df$inhabitants_treshold_factor + df$state + df$year) # FE
  ...
  #summary(rdd)
  rdd_vary_const_type2 <- rdd_vary_const_type2 %>%
    dplyr::add_row(
      const_typ_magistrat = i,
      point_estimate = rdd$coef[1],
      se = rdd$se[1],
      se_robust = rdd$se[3],
      ci_lower = rdd$ci[3,1],
...
rdd_vary_const_type2 <- rdd_vary_const_type2 %>% drop_na()
save(rdd_vary_const_type2, file = "tables_robustness_checks/rdd_vary_const_type2.Rdata")
rm(all_values, data_subset, df, i, rdd)
```
```{r}
load(file = "tables_robustness_checks/rdd_vary_const_type2.Rdata")
rdd_vary_const_type2
```

```{r}
ggplot2::ggplot(data = rdd_vary_const_type2, aes(x = const_typ_magistrat, y = point_estimate)) +
  geom_point(size = 1.5, color = ifelse(rdd_vary_const_type2$p <= p_value_cutoff, 2, 1)) +
  geom_errorbar(aes(ymin = ci_lower, ymax = ci_upper),
                color = ifelse(rdd_vary_const_type2$p <= p_value_cutoff, 2, 1),
                linetype = ifelse(rdd_vary_const_type2$p <= p_value_cutoff, 2, 1),
                width = 0.5) +
  geom_hline(aes(yintercept = 0), linetype = "dotted") +
  scale_y_continuous(labels = scales::percent) +
  labs(y = y_lab_RDD)
...
`const_typ_magistrat == 1` is significant, as expected (reasons see above).
...
for (i in c("fuzzy", "sharp")) {
  ...
  p <- ggplot2::ggplot(data = df, aes(x = reorder(mayor_party_affiliation, c(1,3,2)),
    ...
    geom_errorbar(aes(ymin = ci_lower, ymax = ci_upper),
                  ...
                  linetype = ifelse(df$p <= p_value_cutoff, 2, 1),
                  width = 0.3) +
    geom_text(aes(y = -0.155, label = note),
            colour = ifelse(df$p <= p_value_cutoff, 2, 1), vjust = 0) +
    geom_hline(aes(yintercept = 0), linetype = "dotted") +
    scale_y_continuous(labels = scales::percent) +
    labs(y = y_lab_RDD, title = i)
  print(p)
}
...
data_subset <- data %>% # prepare data set used for analysis
  ...
  drop_na()
table(data_subset$state)
lims <- c(-0.05, 0.04) # ggplot y axis limits
```
```{r, eval=FALSE}
# to do: remove eval=FALSE
...
p_maj1 <-
  ...
  geom_errorbar(aes(ymin = ci_lower, ymax = ci_upper),
                ...
                width = 0.5) +
  geom_hline(aes(yintercept = 0), linetype = "dotted") +
  geom_text(aes(y = -0.0475, label = note),
            colour = ifelse(df$p <= p_value_cutoff, 2, 1), vjust = 0) +
...
## Party affiliation of (absolute) majority party
...
plot_council_majorities <- ggpubr::ggarrange(
  ...
  annotate_figure(
    # See https://github.com/kassambara/ggpubr/issues/78
    left = text_grob(y_lab_RDD, rot = 90)
    )
...
ggplot2::ggplot(data = df, aes(x = exp_category2, y = point_estimate)) +
  ...
  labs(x = NULL, y = y_lab_RDD) +
  geom_text(aes(y = -0.13, label = note),
            colour = ifelse(df$p <= p_value_cutoff, 2, 1), vjust = 0) #+
  #coord_flip()
...
## More visible projects favored?

## Social expenditures favored?
...
p1 <- ggplot2::ggplot(data = df1, aes(x = value, y = point_estimate)) +
  ...
  geom_text(aes(y = -0.14, label = note),
            colour = ifelse(df1$p <= p_value_cutoff, 2, 1), vjust = 0, size = 2.75) +
  labs(subtitle = "Share of women in council",
       x = "Quartile", y = y_lab_RDD)
p1
...
p2 <- ggplot2::ggplot(data = df2, aes(x = value, y = point_estimate)) +
  ...
  scale_y_continuous(limits = lims, labels = scales::percent) +
  geom_text(aes(y = -0.14, label = note),
            colour = ifelse(df2$p <= p_value_cutoff, 2, 1), vjust = 0, size = 2.75) +
  labs(subtitle = "Share of women in the largest party",
       x = "Quartile of the heterogeneity-inducing variable", y = y_lab_RDD)
p2
...
Given the results from the subsample approach, I will also run models nonlinear in the female-representation variables:
...
rdd <- fixest::feols(ln_gross_expenditure_pc ~
                       ...
                       inhabitants_treshold_factor + year + state | # fixed-effect controls
                       # First stage regression:
                       inhabs_rel_to_cutoff*total_seats*poly(female_share_council, 2) ~
                       inhabs_rel_to_cutoff*above_cutoff*poly(female_share_council, 2),
                     vcov = "threeway",
                     data = df)
summary(rdd)
rdd <- fixest::feols(ln_gross_expenditure_pc ~
                       ...
                       inhabitants_treshold_factor + year + state | # fixed-effect controls
                       # First stage regression:
                       inhabs_rel_to_cutoff*total_seats*poly(female_share_largest_party, 2) ~
                       inhabs_rel_to_cutoff*above_cutoff*poly(female_share_largest_party, 2),
                     vcov = "threeway",
                     data = df)
summary(rdd)
rm(df, rdd)
...
Unfortunately, this does not work because "missing values are not allowed in poly".
...
ggplot2::ggplot(data = df, aes(x = threshold, y = point_estimate)) +
  ...
  geom_errorbar(aes(ymin = ci_lower, ymax = ci_upper),
                ...
                linetype = ifelse(df$p <= p_value_cutoff, 2, 1),
                width = 0.1) +
  geom_text(aes(y = -0.6, label = states), size = 2.5, vjust = 1) +
  geom_hline(aes(yintercept = 0), linetype = "dotted") +
  scale_x_log10() +
  scale_y_continuous(limits = c(-1.2, 1.1), labels = scales::percent) +
...
Run these regressions for each state separately:

```{r, size_variation_by_state, eval=FALSE}
# code still needs some work, not ready yet
# to do: remove eval=FALSE
for (j in c(1, 5)) {
  data_th <- data %>% # prepare data set used for analysis
    filter(state == j) %>%
...
all_th <- data_th %>%
  ...
  filter(!(inhabitants_treshold_factor %in% c("45000", "35000", "60000", "75000", "80000", "150000", "2e+05", "250000", "3e+05", "4e+05", "5e+05", "550000", "7e+05"))) %>%
  pull() # vector of thresholds
rdd_vary_th_by_state <- # prepare data set to store the results
  tibble(state = j, threshold = "", point_estimate = "", se = "", se_robust = "",
         ci_lower = "", ci_upper = "", p = "", bw = "", obs_below = "", obs_above = "") %>%
  mutate(across(c(2:last_col()), as.numeric))
for (i in all_th) {
  print(paste("state", j, "thresholds:", i))
  df <- data_th %>%
    filter(inhabitants_treshold_factor == i)
  ...
  #summary(rdd)
  rdd_vary_th_by_state <- rdd_vary_th_by_state %>%
    dplyr::add_row(
      state = j,
      threshold = as.numeric(i),
      point_estimate = rdd$coef[1],
...
rdd_vary_th_by_state <- rdd_vary_th_by_state %>% drop_na() %>% arrange(threshold)
save(rdd_vary_th_by_state, file = "tables_robustness_checks/rdd_vary_th_by_state.Rdata")
rm(all_th, data_th, df, i, rdd, states_in_df)
}
```
```{r, eval=FALSE}
load(file = "tables_robustness_checks/rdd_vary_th_by_state.Rdata")
rdd_vary_th_by_state
```
...
## "Kreisfreie Städte" vs. not

General remark: The two variables `kreisfreie_stadt` and `stadt` are mutually exclusive. If a city is categorized as an independent large city (`kreisfreie_stadt == 1`), `stadt == 1`. So each town belongs to one of three categories:

- `kreisfreie_stadt = 0` & `stadt = 0` -> "Gemeinde"/municiapality
- `kreisfreie_stadt = 0` & `stadt = 1` -> "kreisangehörige Stadt"/town within a county
- `kreisfreie_stadt = 1` & `stadt = 1` -> "kreisfreie Stadt"/independent large city
...
data_subset <- data %>% # prepare data set used for analysis
  ...
  mutate(
    status = case_when(
      ...
      as.numeric(kreisfreie_stadt) == 2 & as.numeric(stadt) == 1 ~ "Independent large city"
    )
  )
table(data_subset$state)
```
...
Extremely high p value for the joint test: No significant differences from baseline (independent large cities).
