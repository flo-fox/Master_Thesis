...
# Hsu & Shen (2019) heterogeneity test
...
df <- data %>% # prepare data set used for analysis
  select(ln_gross_expenditure_pc, inhabs_rel_to_cutoff, total_seats,
...
#save(results, file = "tables_robustness_checks/hsu_shen_het_test.RData")
```
```{r}
...
rdd <- fixest::feols(ln_gross_expenditure_pc ~
                       ...
                       inhabitants_treshold_factor + state + year | # fixed-effect controls
                       # First stage regression:
                       inhabs_rel_to_cutoff*total_seats*ward_elections ~ # Instrument our standard RDD...
                       inhabs_rel_to_cutoff*above_cutoff*ward_elections, # ... with being above the cutoff
                     vcov = "threeway",
                     data = df)
summary(rdd)
#rm(df)
...
to do: implement sharp design (Bavarian data...)
...
rdd_vary_may_affil <- rdd_vary_may_affil %>% drop_na()
...
```{r}
for (i in c("sharp", "fuzzy")) {
  df <- rdd_vary_may_affil %>% filter(method == i)
  p <- ggplot2::ggplot(data = df, aes(x = mayor_party_affiliation, y = point_estimate)) +
    geom_point(size = 1.5, color = ifelse(df$p <= p_value_cutoff, 2, 1)) +
    geom_errorbar(aes(ymin = ci_lower, ymax = ci_upper),
                  color = ifelse(df$p <= p_value_cutoff, 2, 1),
                  linetype = ifelse(df$p <= p_value_cutoff, 2, 1),
...
```{r}
ggplot2::ggplot(data = rdd_vary_abs_majority, aes(x = absolute_majority, y = point_estimate)) +
...
```{r}
data_subset <- data %>% # prepare data set used for analysis
  select(ln_gross_expenditure_pc, inhabs_rel_to_cutoff, total_seats, inhabitants_treshold_factor, state, year, cdu_csu_majority, spd_majority, closest, above_cutoff) %>%
  mutate(abs_majority_party = case_when(
    cdu_csu_majority == 1 ~ "cdu_csu",
    spd_majority == 1 ~ "neither cdu_csu nor spd", # spd only yields error in rdrobust...
...
rdd_vary_abs_majority <- rdd_vary_abs_majority %>% drop_na()
save(rdd_vary_abs_majority, file = "tables_robustness_checks/rdd_vary_abs_majority2.Rdata")
rm(all_values, df, i, rdd)
```
```{r}
...
for (i in exp_category) {
  df <- data %>%
    select((!!sym(i)), inhabs_rel_to_cutoff, total_seats,
...
```{r}
ggplot2::ggplot(data = rdd_vary_exp_category, aes(x = exp_category2, y = point_estimate)) +
  geom_point(size = 1.5, color = ifelse(rdd_vary_exp_category$p <= p_value_cutoff, 2, 1)) +
  geom_errorbar(aes(ymin = ci_lower, ymax = ci_upper),
                color = ifelse(rdd_vary_exp_category$p <= p_value_cutoff, 2, 1),
                linetype = ifelse(rdd_vary_exp_category$p <= p_value_cutoff, 2, 1),
                width = 0.5) +
  geom_hline(aes(yintercept = 0), linetype = "dotted") +
  scale_y_continuous(labels = scales::percent) +
...
# to do: remove eval=FALSE
data_fem_rep <- data_by %>% # prepare data set used for analysis
  ...
  mutate(
    female_share_council = ifelse(female_share_council > mean(data_by$female_share_council), 1, 0),
    #female_share_council = as.factor(female_share_council),
    female_share_largest_party = ifelse(female_share_largest_party >
                                          mean(data_by$female_share_largest_party),
                                        1,
                                        0)#,
    #female_share_largest_party = as.factor(female_share_largest_party)
  ) %>%
  drop_na()
#all_years <- data_year %>% distinct(year) %>% pull() # vector of years
...
```{r}
df <- rdd_vary_fem_rep %>% mutate(x = paste(var, value))
...
rdd_fem_rep1 <- fixest::feols(ln_gross_expenditure_pc ~
                       1  | # controls
                       inhabitants_treshold_factor + year | # fixed-effect controls
                       # First stage regression:
                     ...
                     vcov = "twoway",
                     data = df)
summary(rdd_fem_rep1)
rm(df)
```

I find that the "Law of 1/n" effect decreases in the share of female representatives. Contrary to what I think is generally perceived, women in council tend to decrease spending at the threshold.
...
df <- data_by %>%
  filter(closest <= 0.1) %>%
  mutate(state = as.factor(state))
rdd_fem_rep2 <- fixest::feols(ln_gross_expenditure_pc ~
                       1  | # controls
                       inhabitants_treshold_factor + year | # fixed-effect controls
                       # First stage regression:
                     ...
                     vcov = "twoway",
                     data = df)
summary(rdd_fem_rep2)
rm(df)
```

In general, looking similar here but not significant which is surprising given that we are now looking at the female share of the largest and most important party only...
...
```{r}
ggplot2::ggplot(data = rdd_vary_city, aes(x = status, y = point_estimate)) +
