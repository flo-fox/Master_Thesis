...
data_by <- data %>%
  filter(state == 9) %>%
  select(
    ln_gross_revenue_pc, exact_pop,
    state, year, inhabitants_treshold
    ) %>%
  drop_na()
```
...
rdd <-
  rdmulti::rdms(
    Y = data_by$ln_gross_revenue_pc,
    X = data_by$exact_pop,
    C = data_by$inhabitants_treshold
  )
summary(rdd)
```
```{r, eval = FALSE}
df <- data_by %>% filter(inhabitants_treshold < 100000) # After this threshold, error
rdd <-
  rdmulti::rdms(
    Y = data_by$ln_gross_revenue_pc,
    X = data_by$exact_pop,
    C = sort(unique(data_by$inhabitants_treshold))
  )
summary(rdd)
rm(df)
```

Does not even work for Bavarian sharp case. I have no clue why.
