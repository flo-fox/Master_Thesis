...
etable(rdd_conf_1, rdd_conf_2,
       ...
       # LaTeX settings
       title = "Results of the interacted \\ac{RDD}s to investigate (non)confounded thresholds",
...
ggplot2::ggplot(data = df, aes(x = donut_radius, y = point_estimate)) +
  ...
  geom_errorbar(aes(ymin = ci_lower, ymax = ci_upper),
                ...
                linetype = ifelse(df$p <= 0.05, 2, 1),
                width = 0.25) +
  geom_text(aes(x = donut_radius, y = -0.06, label = note),
            colour = ifelse(df$p <= 0.05, 2, 1), vjust = 0) +
  geom_hline(aes(yintercept = 0), linetype = "dotted") +
  scale_y_continuous(labels = scales::percent) +
...
ggsave(filename = "plots/rdd_donut.pdf", width = 6, height = 6*9/16)
rm(df)
```

Four out of six regressions are positively significant...
